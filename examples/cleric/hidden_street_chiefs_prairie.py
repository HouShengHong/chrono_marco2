...
list_explosion_left: list[str] = [
    alpha_setting.AttackKeys.explosion,
    KeyBinds.left,
]
list_explosion_right: list[str] = [
    alpha_setting.AttackKeys.explosion,
    KeyBinds.right,
]
...
def how_to_play(player: Player):
    match player.eye.status.current_yellow_point_position_in_mini_map:
        ...
        # platform 3
        case (x, y) if 70 <= x <= 130 and 49 <= y <= 63:
            with pyautogui.hold(list_explosion_left):
                for _ in range(5):
                    little_rush.hold()
        ...
        # platform 4
        # case (x, y) if 138 <= x <= 182 and 52 <= y <= 68:
        case (x, y) if 131 <= x <= 182 and 52 <= y <= 68:
...
if __name__ == "__main__":
    ...
    eye: Eye = Eye(
        ...
        MiniMapData.hidden_street_chiefs_prairie["region"],
    )

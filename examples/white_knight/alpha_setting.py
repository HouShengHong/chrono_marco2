...
attack_hold_time:tuple[int, int] = (0.03, 0.06)
attack_end_sleep_time: tuple[int, int] = (0.64, 0.64)
attack_end_sleep_time_20off: tuple[int, int] = (0.52, 0.52)
attack_end_sleep_time_40off: tuple[int, int] = (0.38, 0.38)

attack_prev_jump_hold_time: tuple[int, int] = (0.02, 0.02)
...
class BuffKeepers:
    ...
    magic_crash: BuffKeeper = BuffKeeper(
        ...
        True)
    power_guard: BuffKeeper = BuffKeeper(850, Path(__file__).parent / "keepers" / "power_guard.txt", [KeyHolderWin("8", (0.2, 0.3), (0.6, 0.6))], True)
    booster: BuffKeeper     = BuffKeeper(850, Path(__file__).parent / "keepers" / "booster.txt",     [KeyHolderWin("9", (0.2, 0.3), (0.4, 0.4))], True)
    nimble_feet: BuffKeeper = BuffKeeper(850, Path(__file__).parent / "keepers" / "nimble_feet.txt", [KeyHolderWin("0", (0.2, 0.3), (1.1, 1.1))], True)
...
def action_template(
    ...
    hold_time: tuple[int, int] = (0, 0),
    end_sleep_time: tuple[int, int] = (0, 0),    
) -> KeyHolderWin:
    ...
    action: KeyHolderWin = KeyHolderWin(direction_keys + main_keys + change_element_keys, hold_time, end_sleep_time)
    return action
...
def attack_prev_jump(
    ...
    direction_keys: list[str] | None = None, 
    change_element_keys: list[str] | None = None,
    hold_time: tuple[int, int] = attack_prev_jump_hold_time,
    end_sleep_time: tuple[int, int] = (0, 0),    
) -> KeyHolderWin:
    jump_keys = jump_keys if jump_keys is not None else [KeyBinds.jump]
    return action_template(jump_keys, direction_keys, change_element_keys, hold_time, end_sleep_time)

def normal_attack(
    main_keys: list[str], 
    direction_keys: list[str] | None = None, 
    change_element_keys: list[str] | None = None,
    hold_time: tuple[int, int] = attack_hold_time,
    end_sleep_time: tuple[int, int] = attack_end_sleep_time,
) -> KeyHolderWin:
    return action_template(main_keys, direction_keys, change_element_keys, hold_time, end_sleep_time)
...
def lightning_attack(
    ...
    change_element_keys: list[str] | None = None,
    hold_time: tuple[int, int] = attack_hold_time,
    end_sleep_time: tuple[int, int] = attack_end_sleep_time_20off,
) -> KeyHolderWin:
    return action_template(main_keys, direction_keys, change_element_keys, hold_time, end_sleep_time)
...
def lightning_rush(
    ...
    # change_element_keys: list[str] | None = None,
    hold_time: tuple[int, int] = attack_hold_time,
    end_sleep_time: tuple[int, int] = attack_end_sleep_time_40off,
) -> KeyHolderWin:
    rush_keys = rush_keys if rush_keys is not None else [KeyBinds.rush]
    return action_template(rush_keys, direction_keys, None, hold_time, end_sleep_time)